...
class Actor:
    ...
    def move(self, game_: 'Game', dx: int, dy: int) -> bool:
        ...
        a = [self]
        i = 0
        if game_.get_actor(x, y) is not None:
            if not game_.get_actor(x, y).is_stop() and\
                    not game_.get_actor(x, y).is_push():
                self.x = self.x + dx
                self.y = self.y + dy
                return True
        while 0 <= x < game_.width and 0 <= y < game_.height:
            if game_.get_actor(x, y) is None:
                while i < len(a):
                    a[i].x += dx
                    a[i].y += dy
                    i += 1
                return True
            if not game_.get_actor(x, y).is_push():
                return False
            a.append(game_.get_actor(x, y))
            x = x + dx
            y = y + dy
        return False
...
class Meepo(Character):
    ...
    def copy(self) -> 'Meepo':
        m_p = Meepo(self.x, self.y)
        m_p.image = self.image
        self.copy_flags(m_p)
        return m_p
    ...
    def handle_key_press(self, game_: 'Game') -> Tuple[int, int]:
        ...
        if key_pressed[pygame.K_LEFT]:
            dx -= 1
            self.image = self.walk_left[self.walk_ig]
            if self.walk_ig == 0:
                self.walk_ig += 1
            else:
                self.walk_ig -= 1
            ...
            dx += 1
            self.image = self.walk_right[self.walk_ig]
            if self.walk_ig == 0:
                self.walk_ig += 1
            else:
                self.walk_ig -= 1
            ...
            dy -= 1
            self.image = self.walk_up[self.walk_ig]
            if self.walk_ig == 0:
                self.walk_ig += 1
            else:
                self.walk_ig -= 1
            ...
            dy += 1
            self.image = self.walk_down[self.walk_ig]
            if self.walk_ig == 0:
                self.walk_ig += 1
            else:
                self.walk_ig -= 1
...
class Wall(Character):
    """A class who is the subclass of Character"""

    def __init__(self, x: int, y: int) -> None:
        """
        Initializes the wall Class and load Wall's images.
        """
        super().__init__(x, y)
    ...
    def copy(self) -> "Wall":
        """
        Returns the copy of wall
        """
        w_l = Wall(self.x, self.y)
        self.copy_flags(w_l)
        return w_l
...
class Rock(Character):
    ...
    def __init__(self, x: int, y: int) -> None:
        """
        Initializes the rock Class and load rock's images.
        """
        super().__init__(x, y)
    ...
    def copy(self) -> "Rock":
        """
        Returns the copy of the rock
        """
        r_k = Rock(self.x, self.y)
        self.copy_flags(r_k)
        return r_k
...
class Flag(Character):
    ...
    def __init__(self, x: int, y: int) -> None:
        """
        Initializes the Flag Class and load Flag's images.
        """
        super().__init__(x, y)
    ...
    def copy(self) -> "Flag":
        """
        Returns the copy of the flag
        """
        f_g = Flag(self.x, self.y)
        self.copy_flags(f_g)
        return f_g
...
class Bush(Actor):
    ...
    def __init__(self, x: int, y: int) -> None:
        """
        Initializes the bush Class and load bush's images.
        """
        super().__init__(x, y)
...
class Subject(Block):
    ...
    def __init__(self, x: int, y: int, word_: str) -> None:
        """
        Initializes the subject Class and load subject's images.
        """
        super().__init__(x, y, word_)
        s = word_.lower()
        self.image = load_image(WORDS_SPRITES[s])

    def copy(self) -> "Subject":
        """
        Returns the copy of subject
        """
        s_j = Subject(self.x, self.y, self.word)
        return s_j
...
class Attribute(Block):
    ...
    def __init__(self, x: int, y: int, word_: str) -> None:
        """
        Initializes the attribute Class and load attribute's images.
        """
        super().__init__(x, y, word_)
        s = word_.lower()
        self.image = load_image(WORDS_SPRITES[s])
    ...
    def copy(self) -> "Attribute":
        """
        Returns the copy of attribute
        """
        a_b = Attribute(self.x, self.y, self.word)
        return a_b
...
class Is(Block):
    ...
    def copy(self) -> "Is":
        """
        Returns the copy of is
        """
        i_s = Is(self.x, self.y)
        i_s.image = self.image.copy()
        return i_s
    ...
    def update(self, up: Optional[Actor],
        ...
        if isinstance(up, Subject) and isinstance(down, Attribute):
            v_t = "{}{}{}".format(up.word, " is", down.word)
        if isinstance(left, Subject) and isinstance(right, Attribute):
            h_z = "{}{}{}".format(left.word, " is", right.word)
        if h_z != "" and v_t != "":
            self.image = load_image(IS_DARK_BLUE)
        elif (h_z != "" and v_t == "") or (h_z == "" and v_t != ""):
